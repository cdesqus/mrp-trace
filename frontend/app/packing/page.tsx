...
type SmallBox = {
  ...
  product_code: string; product_name: string; master_box_capacity: number;
    ...
    <ModulePage eyebrow="Logistics & Packing" title="Packing Operations" description="Stage fixed consecutive serial groups, complete pending rework, then lock Small and Master Boxes.">
      ...
      {tab === "small" ? (
        ...
        <div className="grid gap-5 xl:grid-cols-[1fr_360px]">
          ...
          <aside className="card h-fit"><h3 className="font-black">Master Box Summary</h3><dl className="mt-5 space-y-4 text-sm"><div className="flex justify-between"><dt className="text-slate-500">Production Order</dt><dd className="font-bold">{boxes[0]?.production_order_number ?? "—"}</dd></div><div className="flex justify-between"><dt className="text-slate-500">Product</dt><dd className="font-bold">{boxes[0]?.product_code ?? "—"}</dd></div><div className="flex justify-between"><dt className="text-slate-500">Small Boxes</dt><dd className="font-bold">{boxes.length}{capacity?` / ${capacity}`:""}</dd></div><div className="flex justify-between"><dt className="text-slate-500">Total FG</dt><dd className="font-bold">{unitTotal}</dd></div></dl><button className="primary mt-6 w-full" disabled={!capacity||boxes.length!==capacity||busy} onClick={() => void lockMaster()}>{busy ? "Processing…" : boxes.length===capacity&&capacity>0 ? "Lock Master Box" : "Complete Required Scans"}</button><button className="mt-3 w-full rounded-xl py-2.5 text-sm font-bold text-slate-500 hover:bg-slate-50" disabled={!boxes.length || busy} onClick={() => setBoxes((current) => current.slice(0, -1))}>Undo Last Scan</button>{lastMasterBoxLabel&&<button className="mt-3 w-full rounded-xl border border-blue-200 py-2.5 text-sm font-black text-blue-700 hover:bg-blue-50" onClick={()=>setMasterBoxLabel(lastMasterBoxLabel)}>Reprint Last Master Label</button>}</aside>
        </div>
      )}
        ...
        <div className="grid grid-cols-[1fr_150px] gap-4 p-5">
          <div className="min-w-0">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 border-y border-slate-300 py-3 text-sm">
              <div><p className="text-[10px] font-black uppercase text-slate-500">Quantity</p><p className="text-lg font-black">{box.actual_qty} PCS</p></div>
      ...
      <div className="label-print-root mx-auto overflow-hidden rounded-2xl border-2 border-slate-950 bg-white text-slate-950">
        ...
        <div className="grid grid-cols-[1fr_165px] gap-4 p-5">
          <div className="min-w-0">
            ...
            <div className="mt-3 grid grid-cols-3 gap-2 border-y border-slate-300 py-3 text-center"><div><p className="text-[9px] font-black uppercase text-slate-500">Small Boxes</p><p className="text-xl font-black">{label.small_box_count}</p></div><div><p className="text-[9px] font-black uppercase text-slate-500">Total FG</p><p className="text-xl font-black">{label.unit_quantity}</p></div><div><p className="text-[9px] font-black uppercase text-slate-500">Packed</p><p className="text-sm font-black">{new Date(label.packed_at).toLocaleDateString("en-GB")}</p><p className="text-[10px]">{new Date(label.packed_at).toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit"})}</p></div></div>
